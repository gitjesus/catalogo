...
<script>
	$(function()
	{
		$(".btnAsociaProducto").click(
			function()
			{
				var id=$(this).attr("data-id");
				$("#contenido").load("venta.php?id="+id);
			}
		);
	});
</script>
<div id="resultadoPersona" class="panel panel-default">
    		<table class="table striped-table">
    			<thead>
    				<tr>
    					<th>ID</th><th>Nombre</th> <th>Direccion</th> <th>  producto  </th>
    				</tr>
    			</thead>
    			<tbody>
    			<?
    			...
    			?>
    				<tr>
    					<td><?=$persona->id?></td><td class="text-left" nowrap><?=$persona->nombre?></td><td nowrap><?=$persona->direccion?></td><td><input type="button"  value="Agregar Producto" class="btn btn-info btn-sm btnAsociaProducto"  data-id="<?=$persona->id?>" /></td>
